...
export default function About() {
  return (
    <section id="sobre" className="bg-charcoal-800 py-24 md:py-32 overflow-hidden">
      <div className="max-w-6xl mx-auto px-6 md:px-12">

        {/* Top intro */}
        <div className="grid md:grid-cols-2 gap-16 items-center mb-20">
          <div>
            <span className="section-tag text-gold-300 block mb-5">A solução</span>
            <h2 className="font-serif font-300 text-cream-50 text-3xl md:text-4xl lg:text-5xl leading-tight mb-6">
              Dra. Alana Dávila —{' '}
              <em className="italic text-gold-300">sua parceira</em> em
              estética avançada
            </h2>
          ...
          <div className="relative">
            <div className="relative overflow-hidden" style={{ paddingBottom: '110%' }}>
              <img
                src="https://images.unsplash.com/photo-1638202993928-7267aad84c31?w=700&q=80&fit=crop&crop=top"
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {pillars.map((p) => (
            <div key={p.number} className="border border-cream-50/10 p-8 relative group hover:border-gold-400/40 transition-colors duration-300">
              ...
              <h3 className="text-cream-50 font-serif font-300 text-xl mb-3">{p.title}</h3>
              <p className="text-cream-200/50 font-sans font-300 text-sm leading-relaxed">{p.desc}</p>
            </div>
          ))}
        </div>
      </div>
